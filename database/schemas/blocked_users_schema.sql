...
CREATE TABLE IF NOT EXISTS blocked_users (
    ...
    -- Уникальная пара: один пользователь не может заблокировать другого дважды
...
-- Пользователь может видеть только свой черный список
CREATE POLICY "Users can view their own blocked list"
    ON blocked_users FOR SELECT
    USING (auth.uid() = user_id);

-- Пользователь может добавлять в свой черный список
CREATE POLICY "Users can block others"
    ON blocked_users FOR INSERT
    WITH CHECK (auth.uid() = user_id);

-- Пользователь может удалять из своего черного списка
...
-- Триггер для автоматического удаления чатов при блокировке
