-- Защита от DDoS атак и Rate Limiting
...
DECLARE
    v_similar_messages INTEGER;
    v_message_count INTEGER;
BEGIN
    -- Подсчитываем похожие сообщения за последние N минут
    SELECT COUNT(*) INTO v_similar_messages
    FROM messages
    WHERE user_id = p_user_id
    AND created_at >= NOW() - (p_time_window_minutes || ' minutes')::INTERVAL
    AND similarity(content, p_content) > 0.8; -- 80% схожесть
    ...
    -- Если более 5 похожих сообщений или более 50 сообщений за 5 минут
    IF v_similar_messages > 5 OR v_message_count > 50 THEN
        -- Логируем спам
        INSERT INTO suspicious_activity (user_id, activity_type, details, severity)
        VALUES (
            p_user_id,
            'spam_detected',
            jsonb_build_object(
                'similar_messages', v_similar_messages,
                'total_messages', v_message_count,
                'time_window_minutes', p_time_window_minutes
            ),
            CASE 
                WHEN v_message_count > 100 THEN 'critical'
                WHEN v_message_count > 75 THEN 'high'
                ELSE 'medium'
            END
        );
...
-- Триггер для автоматической проверки спама при отправке сообщения
CREATE OR REPLACE FUNCTION check_message_spam()
RETURNS TRIGGER AS $$
BEGIN
    -- Проверяем на спам
    IF detect_spam_pattern(NEW.user_id, NEW.content) THEN
        RAISE EXCEPTION 'Spam detected. Please slow down.';
    END IF;
...
-- Создаем триггер (опционально, можно включить при необходимости)
-- DROP TRIGGER IF EXISTS trigger_check_message_spam ON messages;
-- CREATE TRIGGER trigger_check_message_spam
--     BEFORE INSERT ON messages
--     FOR EACH ROW
--     EXECUTE FUNCTION check_message_spam();

-- Создаем задачу для автоматической очистки (через pg_cron если доступен)
...
COMMENT ON FUNCTION detect_spam_pattern IS 'Обнаружение спам-паттернов в сообщениях';
