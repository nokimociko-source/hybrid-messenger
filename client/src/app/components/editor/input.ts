...
// Matrix-to and markdown plugin imports removed
...
const getInlineNonMarkElement = (node: Element): MentionElement | EmoticonElement | undefined => {
  ...
  if (node.name === 'a') {
    ...
    // testMatrixTo replaced by native logic if needed, but for now we just skip matrix links.
  }
  return undefined;
};
...
export const htmlToEditorInput = (unsafeHtml: string, markdown?: boolean): Descendant[] => {
  ...
  const processText = (partText: string) => {
    return partText; // Markdown previously escaped here
  };
